...
export const googleAuthController =async (req:any,res:any ,next:any)=>{
  ...
  try {
    ...
    // Handle user authentication logic (e.g., save user to database)
    res.status(200).json({ message: 'User authenticated', userId, email });
  } catch (error) {
    next(error);
  }
}